...
| First playable build | Game Studio's smallest running evidence gate: one player action creates one visible consequence. | Does the action visibly change play? |
...
| Presentation build | Evidence that the chosen format earns its cost. | Does the look, camera, input, audio, or UI add player value? |
...
| UX Designer | Onboarding, input, UI clarity, accessibility areas, and player comprehension. | First playable build, demo candidate, public release materials. |
| Art Director | Visual target, asset pipeline, style guide, and production value. | Presentation build and vertical slice. |
| Audio Director | Music, sound effects, ambience, mix, and implementation standards. | Presentation build and vertical slice. |
...
| First Playable Build | One player action, one visible consequence, and captured runtime evidence. | Is the central interaction real in a running build? |

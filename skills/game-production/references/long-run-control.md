...
work to one active claim:

```text
Active playable claim:
Smallest next playable change or evidence step:
Evidence required:
Stop condition or blocker:
```

A document, test, helper script, or research note counts only when it protects
or directly unlocks that build. If no such link exists, work on the game itself
...
| `evidence` | A launch, gameplay, or capture path now proves the active claim. |
| `support` | It directly unblocks the next playable build and is followed by that build. |
...
- After two consecutive support-only material checkpoints, re-anchor before
  ...
  prerequisite and the playable or evidence action it immediately unlocks.
...
- Keep one active claim for the run. Parallelize only independent contributions
  to that same claim, with disjoint write scopes; synthesize their evidence
  before changing the milestone or next build. Sequential execution remains
  valid.
...
(rerun discovery or the evidence check before editing), `retry_once` (one
bounded retry; no
...
Use `complete` only when the active claim or its accepted fallback has been
...
```text
Active claim:
Work type: playable | evidence | support | blocked

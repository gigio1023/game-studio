...
creator recognizes as theirs, production that advances one playable build at a
...
- **A milestone is a playable build, not a task list.** Plan the smallest
  build that retires the biggest uncertainty; never call a prototype a vertical slice.
- **The human owns taste.** Agents propose, compare, implement, and verify;
  ...
  creator.
- **Mechanical evidence is not a playable build.** Files existing, projects
